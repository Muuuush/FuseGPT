...
class Fuser():
    ...
    def __init__(self, layers_2fuse, inps, attention_mask, position_ids, fuse_idx, target_idx, importance_rank, args):
    ...
    def fuse_one_layer(self, layers_origin, args):
        ...
        for i in range(len(layers_origin)):
            if i == self.fuse_idx:
                ...
                continue
            target_idx = i
            lora_rank = int(args.min_lora_rank + self.importance_rank[i] * (args.max_lora_rank - args.min_lora_rank)) if self.importance_rank[i] != -1 else 0
            lora_rank_list.append(lora_rank)
            layers_origin = self.fuse_by_coef(layers_origin, self.fuse_idx, target_idx, lora_rank)
        
        print("lora ranks:", lora_rank_list)
